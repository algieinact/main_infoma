...
<div class="max-w-2xl mx-auto py-10">
    <form
        action="{{ $mode === 'create' ? route('provider.activities.store') : route('provider.activities.update', $activity) }}"
        ...
        <div>
            ...
            <textarea name="description" class="w-full border rounded px-3 py-2" rows="4"
                required>{{ old('description', $mode === 'edit' ? $activity->description : '') }}</textarea>
            @error('description')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <select name="type" class="w-full border rounded px-3 py-2" required>
                ...
                <option value="seminar"
                    {{ old('type', $mode === 'edit' ? $activity->type : '') == 'seminar' ? 'selected' : '' }}>Seminar
                </option>
                <option value="webinar"
                    {{ old('type', $mode === 'edit' ? $activity->type : '') == 'webinar' ? 'selected' : '' }}>Webinar
                </option>
                <option value="mentoring"
                    {{ old('type', $mode === 'edit' ? $activity->type : '') == 'mentoring' ? 'selected' : '' }}>
                    Mentoring</option>
                <option value="lomba"
                    {{ old('type', $mode === 'edit' ? $activity->type : '') == 'lomba' ? 'selected' : '' }}>Lomba
                </option>
                <option value="workshop"
                    {{ old('type', $mode === 'edit' ? $activity->type : '') == 'workshop' ? 'selected' : '' }}>Workshop
                </option>
                <option value="training"
                    {{ old('type', $mode === 'edit' ? $activity->type : '') == 'training' ? 'selected' : '' }}>Training
                </option>
            </select>
            @error('type')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        ...
        <div>
            ...
            <select name="format" class="w-full border rounded px-3 py-2" required>
                ...
                <option value="online"
                    {{ old('format', $mode === 'edit' ? $activity->format : '') == 'online' ? 'selected' : '' }}>Online
                </option>
                <option value="offline"
                    {{ old('format', $mode === 'edit' ? $activity->format : '') == 'offline' ? 'selected' : '' }}>
                    Offline</option>
                <option value="hybrid"
                    {{ old('format', $mode === 'edit' ? $activity->format : '') == 'hybrid' ? 'selected' : '' }}>Hybrid
                </option>
            </select>
            @error('format')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        ...
        <div>
            ...
            <select name="is_free" class="w-full border rounded px-3 py-2" required>
                <option value="1"
                    {{ old('is_free', $mode === 'edit' ? $activity->is_free : '') == 1 ? 'selected' : '' }}>Ya</option>
                <option value="0"
                    {{ old('is_free', $mode === 'edit' ? $activity->is_free : '') == 0 ? 'selected' : '' }}>Tidak
                </option>
            </select>
            @error('is_free')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        ...
        <div>
            ...
            <input type="datetime-local" name="start_date" class="w-full border rounded px-3 py-2"
                value="{{ old('start_date', $mode === 'edit' ? $activity->start_date->format('Y-m-d\TH:i') : '') }}"
                required>
            @error('start_date')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <input type="datetime-local" name="end_date" class="w-full border rounded px-3 py-2"
                value="{{ old('end_date', $mode === 'edit' ? $activity->end_date->format('Y-m-d\TH:i') : '') }}"
                required>
            @error('end_date')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <input type="datetime-local" name="registration_deadline" class="w-full border rounded px-3 py-2"
                value="{{ old('registration_deadline', $mode === 'edit' ? $activity->registration_deadline->format('Y-m-d\TH:i') : '') }}"
                required>
        ...
        <div>
            ...
            <input type="text" name="requirements" class="w-full border rounded px-3 py-2"
                value="{{ old('requirements', $mode === 'edit' ? (is_array($activity->requirements ?? null) ? implode(',', $activity->requirements) : $activity->requirements) : '') }}"
                required>
            @error('requirements')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <input type="text" name="benefits" class="w-full border rounded px-3 py-2"
                value="{{ old('benefits', $mode === 'edit' ? (is_array($activity->benefits ?? null) ? implode(',', $activity->benefits) : $activity->benefits) : '') }}"
                required>
            @error('benefits')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
